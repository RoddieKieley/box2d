...
func (joint *B2PrismaticJoint) Dump() {
	...
	fmt.Printf("  jd.collideConnected = bool(%t);\n", joint.M_collideConnected)
	fmt.Printf("  jd.localAnchorA.Set(%.15ef, %.15ef);\n", joint.M_localAnchorA.X, joint.M_localAnchorA.Y)
	fmt.Printf("  jd.localAnchorB.Set(%.15ef, %.15ef);\n", joint.M_localAnchorB.X, joint.M_localAnchorB.Y)
	fmt.Printf("  jd.localAxisA.Set(%.15ef, %.15ef);\n", joint.M_localXAxisA.X, joint.M_localXAxisA.Y)
	fmt.Printf("  jd.referenceAngle = %.15ef;\n", joint.M_referenceAngle)
	fmt.Printf("  jd.enableLimit = bool(%t);\n", joint.M_enableLimit)
	fmt.Printf("  jd.lowerTranslation = %.15ef;\n", joint.M_lowerTranslation)
	fmt.Printf("  jd.upperTranslation = %.15ef;\n", joint.M_upperTranslation)
	fmt.Printf("  jd.enableMotor = bool(%t);\n", joint.M_enableMotor)
	fmt.Printf("  jd.motorSpeed = %.15ef;\n", joint.M_motorSpeed)
	fmt.Printf("  jd.maxMotorForce = %.15ef;\n", joint.M_maxMotorForce)
